...
import { Observable, map, switchMap, shareReplay, of, from, tap, finalize, Subject, startWith } from 'rxjs';
            ...
            <main class="content w-100">
                ...
                <div class="files-container">
                    ...
                    <ng-container *ngIf="items$ | async as items">
                        <section class="section-folders" *ngIf="getFolders(items).length > 0">
                        ...
                        <div *ngIf="items.length === 0" class="empty-state">
                            ...
                            <button class="btn btn-primary" (click)="showCreateFolderDialog()">
                                Create Folder
                            </button>
                        </div>
                    </ng-container>
...
export class FileListComponent implements OnInit, OnDestroy {
    ...
    private updateBreadcrumbPath(): void {
        this.currentPath$ = !this.currentFolderId
            ? of([])
            : this.fileManager.getItemPath(this.currentFolderId)
        ...
        if (input.files?.length) {
            ...
            this.fileManager
                ...
                .pipe(
                    ...
                    switchMap(() => this.fileManager.getItems(this.currentFolderId || undefined)),
                    tap(response => {
    ...
    showCreateFolderDialog(): void {
        ...
        if (name?.trim()) {
            this.folder.createFolder(name.trim(), this.currentFolderId || undefined).subscribe({
                next: () => this.refreshFileList(),
    ...
    showRenameDialog(item: FileItem): void {
        ...
        if (newName?.trim() && newName !== item.name) {
            this.fileManager.updateItem(item.id, { name: newName.trim() })
                .subscribe({
                    next: () => this.refreshFileList(),
